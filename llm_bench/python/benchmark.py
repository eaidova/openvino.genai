...
def run_text_generation(input_text, num, model, tokenizer, args, iter_data_list, warmup_md5, prompt_index, bench_hook, model_precision, proc_id):
    ...
    result = model.generate(**input_data, max_new_tokens=int(max_output_token_size), min_new_tokens=int(max_output_token_size), num_beams=args['num_beams'], use_cache=True)
